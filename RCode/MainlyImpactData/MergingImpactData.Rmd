...
Dessie%<>%filter(event%in%EQnams)
# Filter IDMC
GIDD%<>%filter(`Hazard Sub Type`=="Earthquake")

SimMerg<-EMDAT%>%dplyr::select(ISO,Continent,Year)
SimMerg<-Dessie%>%dplyr::select(ISO3,date)%>%
  transmute(ISO=ISO3,Continent=convIso3Continent(ISO3),Year=AsYear(date))%>%
  rbind(SimMerg)
SimMerg<-GIDD%>%dplyr::select(ISO3,Year)%>%
