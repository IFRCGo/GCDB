...
p<-out%>%group_by(event_ID,imp_det,imp_type)%>%summarise(intensity=max(intensity),imp_value=max(imp_value))%>%
...
minitax<-taxies%>%filter(list_name=="imp_type")%>%
...
fieldr%<>%filter(!(is.na(event_ID) | is.na(imp_det) | is.na(imp_type)))

fieldr%<>%filter(imp_src_db=="GO-FR")%>%
  arrange(imp_fdate)%>%
  group_by(event_ID,imp_det,imp_type)%>%
  mutate(Count=length(fieldr$event_ID[fieldr$event_ID==unique(event_ID) &
                                      fieldr$imp_det==unique(imp_det) &
                                      fieldr$imp_type==unique(imp_type)]),
         whichmax=which.max(fieldr$imp_value[fieldr$event_ID==unique(event_ID) &
                                                 fieldr$imp_det==unique(imp_det) &
                                                 fieldr$imp_type==unique(imp_type)]))
