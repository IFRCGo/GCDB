...
## IFRC Appeal Data - Earthquake Only

```{r ifrccont,fig.cap="Number of impact records per continent, for earthquakes in the IFRC Emergency Appeal database.",echo=F, results='hide', warning=F, message=F}

appeal<-readRDS("../../RawData/MostlyImpactData/IFRC/appeal.Rdata")

ggplot(appeal)+geom_bar(aes(Continent,fill=Continent),colour="black")+
xlab("Continent")+ylab("Number of Unmatched Impact Recordings")+ggtitle("IFRC-GO Emergency Appeals")+
theme(axis.text.x = element_text(angle = 45, hjust=1))
```

```{r ifrcyear,fig.cap="Number of impact records per year, for earthquakes in the IFRC Emergency Appeal database.",echo=F, results='hide', warning=F, message=F}

ggplot(appeal)+geom_bar(aes(AsYear(imp_sdate)),fill="blue",colour="black")+
xlab("Year")+ylab("Number of Unmatched Impact Recordings")+ggtitle("IFRC-GO Emergency Appeals")+
theme(plot.title = element_text(hjust = 0.5))
```


```{r ifrcappimp, fig.cap="Number of impact records per impact, for earthquakes in the IFRC Emergency Appeal database.",echo=F, results='hide', warning=F, message=F}
