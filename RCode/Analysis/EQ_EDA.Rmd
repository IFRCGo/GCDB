...
Despite this, we can make the exact same plot but for only the impact data that was matched to a hazard footprint. Figure \@ref(fig:impacts_mat) shows the number of recorded events that have been matched with an associated hazard footprint. The number of earthquake events with matched hazard-impact data now reflects that deaths is the most preminent impact in th GCDB.

```{r impacts_mat, fig.cap="Number of impact records per impact, for earthquakes in the GCDB database.",echo=F, results='hide', warning=F, message=F}
...
pvallie<-as.character(ifelse(pval(lm(intensity ~ magnitude + 0,out))==0,"< 2 x 10^(-16)",pval(lm(intensity ~ magnitude + 0,out))))
radj<-summary(lm(intensity ~ magnitude + 0,out))$adj.r.squared

```
We can also compare the earthquake magnitude (on the Richter scale), to the earthquake intensity (MMI), see figure \@ref(fig:intmag). There is a strong correlation between the two values (p-value `r pvallie`).
...
```{r intbuilddam, fig.cap="Disaster aid allocated against earthquake intensity (MMI) for the matched hazards in the GCDB",echo=F, results='hide', warning=F, message=F}
